...
private:
	...
	void BuildPolygonSides(kmldom::FolderPtr airspaceFolder, std::string airspaceClass, Coord coordinate1, Coord coordinate2,
			double floorAltitude, double topAltitude);
	...
	std::string CreateDescription(std::string airspaceClass, float floorLimitM, float topLimitM,
			double floorLimitAglM = NAN, double ceilingLimitAgl = NAN);
...
public:
	...
	void AddAirspace(std::string channel,
		...
		double topAltitude,
		double floorAltitudeAgl = NAN,
		double ceilingAltitudeAgl = NAN);
};
